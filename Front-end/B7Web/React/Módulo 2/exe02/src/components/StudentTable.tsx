            ...
            <tbody>
                {students.map(item => (
                    <tr key={item.id} className="text-gray-800 bg-gray-400 border-b border-gray-600">
                        ...
                        <td className="p3">{item.grade1.toFixed(1)}</td>
                        <td className="p3">{item.grade2.toFixed(1)}</td>
                        <td className="p3 font-bold">{item.active ? ((item.grade1 + item.grade2) / 2).toFixed(1) : '--'}</td>
                    </tr>
                ))}
            </tbody>
        </table>
